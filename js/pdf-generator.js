...
class PDFGenerator {
    ...
    async addFooter() {
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            // Simple footer with better spacing
            this.pdf.setFontSize(8);
            this.pdf.setTextColor(0, 0, 0);
            this.pdf.text(`Page ${i} of ${pageCount}`, this.margin, this.pageHeight - 15);
            this.pdf.text(`Generated by Sri Mukkanneshwara Associate`, 190 - 60, this.pageHeight - 15);
            
            // Add official seal
            await this.addOfficialSeal();
        }
    }

    async addOfficialSeal() {
        // Position seal in bottom right corner with proper spacing
        const sealX = 170; // Right side
        const sealY = this.pageHeight - 50; // Moved up to avoid overlap
        
        try {
            // Load the official seal image
            const sealImage = await this.loadSealImage();
            
            if (sealImage) {
                // Add the actual seal image
                const sealSize = 40; // 40mm diameter
                this.pdf.addImage(sealImage, 'PNG', sealX - 20, sealY - 20, sealSize, sealSize);
            } else {
                // Fallback to text-based seal if image fails to load
                this.addTextBasedSeal(sealX, sealY);
            }
        } catch (error) {
            console.error('Error loading seal image:', error);
            // Fallback to text-based seal
            this.addTextBasedSeal(sealX, sealY);
        }
        
        // Add "OFFICIAL SEAL" text below the seal with proper spacing
        this.pdf.setFontSize(5);
        this.pdf.setFont(undefined, 'bold');
        this.pdf.setTextColor(0, 0, 0);
        this.pdf.text('OFFICIAL SEAL', sealX - 5, sealY + 22);
        
        // Add date and signature line with better spacing
        this.pdf.setFontSize(4);
        this.pdf.setFont(undefined, 'normal');
        this.pdf.text('Date:', sealX - 6, sealY + 25);
        this.pdf.text(new Date().toLocaleDateString('en-IN'), sealX - 6, sealY + 27);
        
        // Signature line
        this.pdf.setLineWidth(0.4);
        this.pdf.line(sealX - 12, sealY + 30, sealX + 12, sealY + 30);
        this.pdf.setFontSize(3.5);
        this.pdf.text('Authorized Signature', sealX - 5, sealY + 33);
    }
    
    async loadSealImage() {
        return new Promise((resolve, reject) => {
            // Create an image element to load the seal
            const img = new Image();
            img.crossOrigin = 'anonymous'; // Allow cross-origin for local files
            ...
            // Load the seal image from the images folder
            img.src = 'images/official-seal.png';
        });
    }
    
    addTextBasedSeal(sealX, sealY) {
        // Fallback text-based seal if image fails to load
        ...
        this.pdf.setFont(undefined, 'bold');
        this.pdf.setTextColor(0, 0, 139);
        this.pdf.text('SRI MUKKANESHWARA', sealX - 9, sealY - 14);
